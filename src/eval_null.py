...
class Generate_Dataset(Dataset):
    def __init__(self, path, content, sub_root):
        ...
        if content == 'coco':
            ...
            self.texts = [' '.join(os.path.splitext(x.split('/')[-1])[0]  # 去掉 .png
                .replace(content, ' ')                  # 删除 content
                .split('_')[:-1]                       # 去掉编号
            ).strip() for x in self.images]
            # print(self.texts)
...
class CLIP_Score():
    def __init__(self, version='openai/clip-vit-large-patch14', device='cuda' if torch.cuda.is_available() else 'cpu'):
        local_model_path = "/home/lzh/xpz/model_weight/CLIP/models--openai--clip-vit-large-patch14/snapshots/main"
...
if __name__ == '__main__':
    ...
    print(f"[NULL] Caculating CLIP Score and FID for {root_paths}... \n")
    ...
    for root_path in root_paths:
        try:
            ...
            for content in tqdm(contents):
                # if f"[NULL] {content}: CS is" in txt_content: continue
                ...
                FIDELITY = torch_fidelity.calculate_metrics(
                    ...
                    input2=os.path.join(args.pretrained_path), 
                    cuda=True, 
                    fid=True, 
                    verbose=False,
                )
                with open(save_txt, 'a') as f:
                    f.writelines(f"[NULL] {content}: CS is {CS * 100}, FID is {FIDELITY['frechet_inception_distance']} \n")
                print(f"[NULL] {content}: CS is {CS * 100}, FID is {FIDELITY['frechet_inception_distance']}")
